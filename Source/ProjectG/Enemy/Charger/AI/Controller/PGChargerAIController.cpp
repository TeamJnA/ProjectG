...
bool APGChargerAIController::CanChargeToLocation(FVector TargetLoc, float Tolerance)
{
	...
	//FVector NavHitLoc;
	//const bool bNavHit = NavSystem->NavigationRaycast(Charger, StartLoc, TargetLoc, NavHitLoc, NULL, this);
	//if (!bNavHit)
	//{
	//	return true; // 길 뚫림 -> 돌진 가능
	//}
	//// =========================================================================
	//// [장애물 검사] ChargeCheck 채널
	//// =========================================================================

	//// 오차 범위
	//if (FVector::DistSquared(NavHitLoc, TargetLoc) < (Tolerance * Tolerance))
	//{
	//	return true;
	//}
	...
	//FVector Direction = (TargetLoc - LineTraceStartLoc).GetSafeNormal();

	//// NavMesh 끊긴 지점보다 조금 더 뒤
	//FVector TraceEnd = NavHitLoc + (Direction * 100.0f);

	//ECollisionChannel ChargeCheckChannel = ECC_GameTraceChannel5;
	...
	//	Params
	//);
	//DrawDebugLine(GetWorld(), LineTraceStartLoc, TraceEnd, FColor::Cyan, false, -1.f, 0, 2.0f);

	//if (bObstacleHit)
	//{
	//	// ChargeCheck 채널에 막힘 (벽, 기둥)
	//	// -> 물리적인 벽이 있어서 NavMesh가 끊긴 것 -> 돌진
	//	return true;
	//}
	//else
	//{
	//	// ChargeCheck 채널을 통과함 (난간 Ignore -> 허공)
	//	// -> 난간 뒤에 벽이 없음(허공) -> Adjusting
	//	return false;
	//}
}